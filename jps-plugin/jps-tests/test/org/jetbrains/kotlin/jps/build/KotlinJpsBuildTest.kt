...
open class KotlinJpsBuildTest : AbstractKotlinJpsBuildTestCase() {
    ...
    fun testCheckIsCancelledIsCalledOftenEnough() {
        ...
        val compileTimeMs = (endTime - startTime).toDouble()
        val averageTimeBetweenChecks = compileTimeMs / checkCancelledCalledCount
        val averageTimeExpectedLimit = compileTimeMs * 0.01
        assert(averageTimeBetweenChecks < averageTimeExpectedLimit) {
            "Average time between isCancelled calls should be less than $averageTimeExpectedLimit, but was $averageTimeBetweenChecks" }
    }
