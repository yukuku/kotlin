...
tasks.create("checkCompileClasspath").doLast {
    ...
    project(":kotlin-reflect").configurations.findByName("archives")?.forEach { println(it) }

    val shadowJars = hashSetOf<File>()
    for (project in rootProject.allprojects) {
        for (task in project.tasks) {
            if (task is ShadowJar) {
                shadowJars.add(File(task.archivePath))
            } else if (task is ProGuardTask) {
